...
namespace Orchard.Environment {
	public class DefaultOrchardHost : IOrchardHost, IShellSettingsManagerEventHandler, IShellDescriptorManagerEventHandler {
		...
		void CreateAndActivateShells() {
			...
			if (allSettings.Any()) {
				foreach (var settings in allSettings) {
					try {
						var context = CreateShellContext(settings);//SellContextFactory
		...
		private void ActivateShell(ShellContext context) {
			...
			context.Shell.Activate();//这边貌似很重要

			_shellContexts = (_shellContexts ?? Enumerable.Empty<ShellContext>()).Union(new [] {context});
			_runningShellTable.Add(context.Settings);//把Settings加入runningShellTable
		}
